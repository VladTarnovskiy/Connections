...
export class CardsEffects {
  ...
  fetchCards$ = createEffect(() => this.actions$.pipe(
    ofType(CardsActions.FetchCards),
    exhaustMap(({ searchValue }) => this.userService.getCards(searchValue).pipe(
  ...
  changePage$ = createEffect(() => this.actions$.pipe(
    ...
    exhaustMap(({ pageToken, searchValue }) => this.userService.getCards(searchValue, pageToken).pipe(
      map((cardsInfo) => CardsActions.FetchCardsSuccess({ cardsInfo })),
      catchError((error: HttpErrorResponse) => {
        const handleError = this.userService.handleError(error);
        return of(CardsActions.FetchCardsFailed({ error: handleError }));
